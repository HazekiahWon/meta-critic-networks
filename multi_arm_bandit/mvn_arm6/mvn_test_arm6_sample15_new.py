...
def main():
    ...
    if os.path.exists("meta_value_network_arm6.pkl"):
        meta_value_network.load_state_dict(torch.load("meta_value_network_arm6.pkl"))
        print("load meta value network success")
    if os.path.exists("task_config_network_arm6.pkl"):
        task_config_network.load_state_dict(torch.load("task_config_network_arm6.pkl"))
    ...
    task_probs = json.load(open("tasks_arm6.json"))
    ...
    for episode in range(TASK_NUMS):
        ...
        print(("aver_reward",np.mean(aver_rewards),"correct prob:",np.mean(correct_prob),"task:",task.probs))

    save_to_json('mvn_arm_6_sample_15.json', results)
